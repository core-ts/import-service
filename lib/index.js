...
var resources = (function () {
  ...
  resources.parseNumber = function (res, key, v) {
    if (v && !isNaN(v)) {
      var n = parseFloat(v);
      res[key] = n;
    }
  };
  resources.parseDate = function (res, key, v) {
    var d = new Date(v);
    if (d instanceof Date && !isNaN(d.valueOf())) {
      res[key] = d;
    }
  };
  resources.parseBool = function (res, key, v) {
    if (v.length > 0) {
      if (v === "1" || v === "Y" || v === "T") {
        res[key] = true;
      }
...
var CSVTransformer = (function () {
  function CSVTransformer(attrs) {
    this.attrs = attrs;
    this.transform = this.transform.bind(this);
    this.parse = this.parse.bind(this);
    this.fieldParser = createFieldParsers(attrs);
  }
  CSVTransformer.prototype.parse = function (data) {
    return this.transform(data);
  };
  CSVTransformer.prototype.transform = function (data) {
    var res = {};
    var l = Math.min(data.length, this.fieldParser.length);
    for (var i = 0; i < l; i++) {
...
exports.CSVTransformer = CSVTransformer;
...
var FixedLengthTransformer = (function () {
  ...
  FixedLengthTransformer.prototype.transform = function (data) {
    ...
    var res = {};
    var i = 0;
    var l = this.fieldParser.length;
    for (var i_1 = 0; i_1 < l; i_1++) {
      var len = this.fieldParser[i_1].length ? this.fieldParser[i_1].length : 10;
      var v = data.substring(i_1, i_1 + len);
      this.fieldParser[i_1].parse(res, this.fieldParser[i_1].name, v.trim());
    }
    return Promise.resolve(res);
  };
